...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITextFieldDelegate {
    ...
    /*
     These are the delagete methods for the UIImagePickerController
     Here, we define what the image picker controller can do.
     */
    
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        
        if let image = info[UIImagePickerController.InfoKey.originalImage] as? UIImage{
            // pick image and enable the shareButton if all the conditions are met
    ...
    /*
     The following lines of code will be the  delagate methods for the UITextField.
     Here we set the behavior of the text field assigned to this delegae.
     */
    ...
    // picking an image from library and camera will have same code and the only difference is the sourceType. Instead of repeating the same code twice, we can create a method that takes the source type as a parameter and then go ahead to present the imagePickerControlller with the specified source typr
